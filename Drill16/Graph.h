...
class Shape {
public:
	void draw() const; // rajzolást fogja végezni 	

	virtual void move(int dx, int dy); // objektumok mozgatása, 2 paraméter amivel mozgatjuk 


	void set_color(Color col) { lcolor = col; } //amivel be tudjuk állítani a vonal színét

	Color color() const { return lcolor; } // ezzel tudjuk elérni a színt 

	void set_style(Line_style sty) { ls = sty; } //stílust állítunk be 
	Line_style style() const { return ls; }

	void set_fill_color(Color col) { fcolor = col; } // kitöltött szín
	Color fill_color() const { return fcolor; }

	Point point(int i) const { return points[i]; } //egy point függvény melyel az i.-edik tagot 
	//le tudjuk kérni 


	int number_of_points() const { return int(points.size()); } /*Hány pontunk van*/

/*Hogy ne lehessen másolni letíltjuk a konstruktort és a másoló értékadást */
	Shape(const Shape&) = delete;
	Shape& operator=(const Shape&) = delete;

	virtual ~Shape() { }

protected:  /*vagyis kivülről nem elérhető*/
	Shape() {}
	/*Pontok listáját is tudjam kezelni, a pontokon végigmegyünk 
	és hívjuk az add függvényt és hozzáadja a Points vektorhoz*/
	Shape(initializer_list<Point> lst) { for (Point p : lst) add(p); }
	/*Itt adja hozzá */
	void add(Point p) { points.push_back(p); }
	void set_point(int i, Point p) { points[i] = p; }
	/*be tudunk állítani egy pontot p re */
	virtual void draw_lines() const; 

private:
	vector<Point> points; //sima pontokat tárolunk benne 
	Color lcolor {fl_color()}; //ez lesz a vonalnak a színe // elkéri az alapértelemezett 
	//szín beállítását 

	Line_style ls {0};
	Color fcolor {Color::invisible}; //kitöltött szín 
};
