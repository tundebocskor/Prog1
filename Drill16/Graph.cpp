...
{ /*adunk neki egy alap rajzolót*/
/*csak akkor rajzolunk ha látható, és több pontunk van 

*/
	if (color().visibility() && 1<points.size())
		for(int i = 1; i < points.size(); ++i)
			/*Vonal kirajzoló függvénye */
			fl_line(points[i-1].x, points[i-1].y, points[i].x, points[i].y);
}
/*Kell paraméter, hogy mennyivel mozgatunk el 
És elmozgatjuk a megadott paraméterekkel 
Leszármazott osztályok is ezt hívják */
...
{/*eltároljuk az alapértelmezett színt , stílust */
	Fl_Color oldc = fl_color();
	fl_color(lcolor.as_int()); 
	fl_line_style(ls.style(), ls.width());
	draw_lines(); /*aktuális típusú draw linest hivja meg, ha kör akkor a körét ha 3szög akkor azt ...*/
